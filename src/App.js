...
function App() {
  const [colorArray, setColorArray] = useState(
    loadLocalStorage("colorSaverArray") ?? colorCodes
  );
  ...
  function addColorCard(newColor) {
    ...
    fetch(apiURL)
      ...
      .then((data) => {
        try {
          setColorArray([
            { id: nanoid(), color: newColor, name: data.name.value },
            ...colorArray,
          ]);
  ...
  function deleteColorCard(event, cardId) {
    setColorArray(
      colorArray.filter((card) => (cardId === card.id ? "" : card))
    );
    ...
    fetch(apiURL)
      ...
      .then((data) => {
        setColorArray(
          colorArray.map((card) =>
            cardId === card.id
              ? { id: card.id, color: newColor, name: data.name.value }
              : card
          )
        );
      });
  ...
  return (
    <div className="App">
      ...
      <div className="color-container">
        <ColorCard
          colors={colorArray}
          onHandleDelete={deleteColorCard}
          onHandleEdit={editColorCard}
        />
      </div>
    </div>
  );
