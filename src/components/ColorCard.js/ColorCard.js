...
export function ColorCard({ colors, onHandleDelete, onHandleEdit }) {
  ...
  //Funktion von MDN übernommen: https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Animations/Tips
  ...
  return colors.map((entry) => {
    return (
      <button
        className="color-container__card"
        onClick={(event) => copyCode(event, entry.color)}
        style={{ backgroundColor: entry.color }}
      >
        <p className="colorName">{entry.name}</p>
        <input
          type="text"
          className="colorText-input"
          value={entry.color}
          onInput={(event) => onHandleEdit(event, entry.id, event.target.value)}
          maxLength="7"
          minLength="4"
